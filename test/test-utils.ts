...
describe('MathUI utils', function () {
    var obj: MathUI.Dictionary<string> = { 'a': 'b', 'c': 'd', 'e': 'f' };
    var fakeArray: MathUI.List<string> = { length: 3, 0: 'a', 1: 'b', 2: 'c', 'other': 'stuff' };
    describe('each', function () {
        it('native array', function () {
            var result = [];
            _.each(['a', 'b', 'c'], (value: string, k: number) => {
        ...
        it('array-like object', function () {
            var result = [];
            _.each(fakeArray, (value: string, k: number) => {
                result.push({ value: value, index: k });
            });
            expect(result).to.eql([{ value: 'a', index: 0 }, { value: 'b', index: 1 }, { value: 'c', index: 2 }]);
        });
        it('object', function () {
            var result = {};
            _.each(obj, (value: string, key: string) => {
                result[key] = value;
            });
    ...
    describe('map', function () {
        it('array', function () {
            var result = _.map([2, 3, 4, 5], (value: number, index: number) => value * (index + 1));
            expect(result).to.eql([1*2, 2*3, 3*4, 4*5]);
        });
        it('object', function () {
            var L = _.map(obj, (value: string, key: string) => [key, value]);
            var result = {};
    ...
    describe('filter', function () {
        it('array', function () {
            var result = _.filter([1, 2, 3, 4], (value: number, index: number) => value % 2 === 0);
            expect(result).to.eql([2, 4]);
        });
        it('object', function () {
            var result = _.filter(obj, (value: string, key: string) => key === 'a' || key === 'e');
